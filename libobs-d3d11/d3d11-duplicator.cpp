...
static const char *dxgi_format_to_name(DXGI_FORMAT format)
{
	switch (format) {
						 ...
						 int monitor_idx)
{
	...
	try {
		...
		blog(LOG_WARNING, "device_duplicator_create: %s", error);
		return nullptr;

	} catch (const HRError &error) {
		blog(LOG_WARNING, "device_duplicator_create: %s (%08lX)",
		     error.str, error.hr);
		return nullptr;
	}
...
static inline void copy_texture(gs_duplicator_t *d, ID3D11Texture2D *tex)
{
	...
	if (!d->texture || (d->texture->width != desc.Width) ||
		...
		if (d->texture && !d->logged_texture) {
			blog(LOG_INFO,
			     "gs_duplicator_update_frame: texture ready "
			     "%ux%u dxgi=%s(%d) gs=%d generalized=%d",
			     desc.Width, desc.Height,
			     dxgi_format_to_name(desc.Format), (int)desc.Format,
			     (int)format, (int)general_format);
			d->logged_texture = true;
		} else if (!d->texture) {
			blog(LOG_WARNING,
			     "gs_duplicator_update_frame: failed to create "
			     "copy texture %ux%u dxgi=%s(%d) gs=%d "
			     "generalized=%d",
			     desc.Width, desc.Height,
			     dxgi_format_to_name(desc.Format), (int)desc.Format,
			     (int)format, (int)general_format);
