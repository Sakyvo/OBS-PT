...
static void resolve_install_root(struct dstr *out)
{
	char *ptr;
	wchar_t path_utf16[MAX_PATH];
	GetModuleFileNameW(NULL, path_utf16, MAX_PATH);
	os_wcs_to_utf8_ptr(path_utf16, 0, &ptr);
	dstr_init_move_array(out, ptr);
	dstr_replace(out, "\\", "/");
	for (int i = 0; i < 3; i++) {
		char *slash = strrchr(out->array, '/');
		if (!slash)
			break;
		dstr_resize(out, slash - out->array);
	}
}
...
int get_portable_path(char *dst, size_t size, const char *name)
{
	...
	if (path.len + 1 <= size) {
		memcpy(dst, path.array, path.len + 1);
		ret = (int)path.len;
	} else if (size > 0) {
		memcpy(dst, path.array, size - 1);
		dst[size - 1] = '\0';
	}
...
char *get_portable_path_ptr(const char *name)
{
	struct dstr path = {0};
	resolve_install_root(&path);
	if (name && *name) {
