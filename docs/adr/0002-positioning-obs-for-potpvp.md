...
>
> **更新 2026-06-21 (task 06-19-installer-package)**：首发安装包按用户显式要求使用 `jim_nvenc` 的 `preset=hp`（zh-CN“最大性能”）+ `bf=0`，不再写当前编码器不读取的 `preset2/tune/multipass`。NVIDIA 610.47 上的 `hp` 兼容风险保留为 smoke-test 风险，不改变默认定位。
>
> **更新 2026-06-22 (task 06-22-modernize-nvenc-presets)**：NVENC 路径已回移植 OBS 28.1.0 的现代 P1-P7 preset / tuning / multipass 模型，610.47 风险由 legacy preset 兼容问题转为已修复项。首发默认改为高版本兼容基线：`preset2=p5`、`tune=hq`、`multipass=qres`、`bf=2`、`psycho_aq=true`。
>
> **更新 2026-06-23**：发行默认配置同步当前 PotPvP Profile：`preset2=p1`、`tune=hq`、`multipass=disabled`、`bf=0`、`psycho_aq=false`；新建 Profile 也复制 PotPvP 模板，不再进入上游自动配置向导。

OBS-PT 的对外定位明确为"Minecraft Java 1.7.10 / 1.8.9 PotPvP 录制玩家的定制 OBS 分支"，所有默认值——480/1 Fractional FPS、跟随显示器分辨率、NVENC H.264 优先（无 HEVC）、按分辨率自适应 CQP、现代 NVENC `preset2=p1` + `tune=hq` + `multipass=disabled` + `bf=0`、传统 MP4 容器、单轨混音、输出到 `<Install Root>/recordings/`——均围绕 "High-FPS Source（MC 客户端）→ Frame Blending 后期工作流 → Bilibili Distribution Ceiling（约 20 Mbps）" 这条工作流链路收敛。en-US / zh-CN / zh-TW / ja-JP / ko-KR 五语言支持是 i18n 实施约束（宣传触达 + 翻译工作量），不是用户群定位。
...
- Frame Blending 工作流不熟悉的读者看到高 CQP + 高 FPS + 现代 NVENC p1/hq/disabled 组合会想"为什么不是 cinematic 标准的 CQP 18 / 更慢 preset"——本 ADR + [[Distribution Ceiling]] 是统一答案。
