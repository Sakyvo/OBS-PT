...
struct duplicator_capture {
	...
	bool capture_cursor;
	bool showing;
	bool wgc_exports_loaded;
	LONG logged_width;
	LONG logged_height;
	bool log_dxgi_attempted;
	bool log_dxgi_created;
	bool log_dxgi_failed;
	bool log_dxgi_update_failed;
	bool log_dxgi_waiting_texture;
	bool log_dxgi_texture_ready;
	bool log_render_no_duplicator;
	bool log_render_no_texture;
	bool log_wgc_unavailable;
	bool log_wgc_init_failed;
	bool log_wgc_created;
	bool log_wgc_inactive;
			 ...
			 LONG width, LONG height)
{
	info("update settings:\n"
	     ...
	     get_method_name(capture->method), capture->dxgi_index,
	     capture->handle);
}

static void reset_diagnostic_logs(struct duplicator_capture *capture)
{
	capture->log_dxgi_attempted = false;
	capture->log_dxgi_created = false;
	capture->log_dxgi_failed = false;
	capture->log_dxgi_update_failed = false;
	capture->log_dxgi_waiting_texture = false;
	capture->log_dxgi_texture_ready = false;
	capture->log_render_no_duplicator = false;
	capture->log_render_no_texture = false;
	capture->log_wgc_unavailable = false;
	capture->log_wgc_init_failed = false;
	capture->log_wgc_created = false;
	capture->log_wgc_inactive = false;
}
				       ...
				       obs_source_t *source)
{
	...
	if (graphics_uses_d3d11) {
		...
		if (capture->winrt_module) {
			capture->wgc_exports_loaded = load_winrt_imports(
				&capture->exports, capture->winrt_module,
				module);
		} else {
			warn("WGC module '%s' was not loaded", module);
		}
	}
...
static void reset_capture_data(struct duplicator_capture *capture)
{
	...
	if (texture && !capture->log_dxgi_texture_ready) {
		info("DXGI texture ready: %ux%u at (%ld,%ld), rotation=%d",
		     capture->width, capture->height, capture->x, capture->y,
		     capture->rot);
		capture->log_dxgi_texture_ready = true;
	} else if (!texture && !capture->log_dxgi_waiting_texture) {
		info("DXGI frame update returned without a texture; waiting "
		     "for first desktop frame");
		capture->log_dxgi_waiting_texture = true;
	}
}
...
static void duplicator_capture_tick(void *data, float seconds)
{
	...
	if (capture->method == METHOD_WGC) {
		if (!capture->wgc_exports_loaded) {
			if (!capture->log_wgc_unavailable) {
				warn("WGC capture unavailable: libobs-winrt "
				     "exports are not loaded");
				capture->log_wgc_unavailable = true;
			}
			goto tick_done;
		}
				...
				capture->capture_winrt =
					capture->exports
						.winrt_capture_init_monitor(
							...
							capture->handle);
				if (capture->capture_winrt) {
					if (!capture->log_wgc_created) {
						info("WGC capture initialized "
						     "for monitor handle %p",
						     capture->handle);
						capture->log_wgc_created = true;
					}
				} else if (!capture->log_wgc_init_failed) {
					warn("WGC capture initialization "
					     "returned null for monitor handle "
					     "%p",
					     capture->handle);
					capture->log_wgc_init_failed = true;
				}
		...
		if (!capture->duplicator) {
			...
			if (capture->reset_timeout >= RESET_INTERVAL_SEC) {
				if (!capture->log_dxgi_attempted) {
					info("creating DXGI duplicator for "
					     "monitor index %d",
					     capture->dxgi_index);
					capture->log_dxgi_attempted = true;
				}

				capture->duplicator = gs_duplicator_create(
					capture->dxgi_index);
				if (capture->duplicator) {
					if (!capture->log_dxgi_created) {
						info("DXGI duplicator created "
						     "for monitor index %d",
						     capture->dxgi_index);
						capture->log_dxgi_created = true;
					}
				} else if (!capture->log_dxgi_failed) {
					warn("DXGI duplicator creation failed "
					     "for monitor index %d",
					     capture->dxgi_index);
					capture->log_dxgi_failed = true;
				}
		...
		if (capture->duplicator) {
			...
			if (!gs_duplicator_update_frame(capture->duplicator)) {
				if (!capture->log_dxgi_update_failed) {
					warn("DXGI frame update failed; "
					     "resetting capture data");
					capture->log_dxgi_update_failed = true;
				}
				free_capture_data(capture);
	...
	if (capture->method == METHOD_WGC) {
		if (capture->capture_winrt) {
			if (capture->exports.winrt_capture_active(
				...
				if (!capture->log_wgc_inactive) {
					warn("WGC capture became inactive");
					capture->log_wgc_inactive = true;
				}
				capture->exports.winrt_capture_free(
					capture->capture_winrt);
				capture->capture_winrt = NULL;
		...
		if (!capture->duplicator) {
			if (!capture->log_render_no_duplicator) {
				warn("render skipped: DXGI duplicator is not "
				     "initialized");
				capture->log_render_no_duplicator = true;
			}
			return;
		}
		...
		if (!texture) {
			if (!capture->log_render_no_texture) {
				warn("render skipped: DXGI duplicator has no "
				     "texture");
				capture->log_render_no_texture = true;
			}
			return;
		}
