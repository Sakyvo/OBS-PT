			 ...
			 bool psycho_aq)
{
	...
	if (obs_data_has_user_value(settings, "preset") &&
		...
		av_opt_set(enc->context->priv_data, "preset", preset2, 0);
		av_opt_set(enc->context->priv_data, "tune", tuning, 0);
		av_opt_set(enc->context->priv_data, "multipass", multipass, 0);
	}
	...
	if (astrcmpi(rc, "cqp") == 0) {
		...
		av_opt_set(enc->context->priv_data, "tune", "lossless", 0);
		av_opt_set(enc->context->priv_data, "multipass", "disabled", 0);
	...
	info("settings:\n"
	     ...
	     "\tpsycho-aq:    %d\n"
	     "\tGPU:          %d\n",
	     rc, bitrate, cqp, enc->context->gop_size, preset2, tuning,
	     multipass, profile, enc->context->width, enc->context->height,
	     enc->context->max_b_frames, psycho_aq, gpu);
...
void nvenc_defaults(obs_data_t *settings)
{
	...
	obs_data_set_default_string(settings, "preset2", "p1");
	obs_data_set_default_string(settings, "multipass", "disabled");
