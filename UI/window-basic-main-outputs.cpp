...
#define HYBRID_MP4_FORMAT "hybrid_mp4"

static inline bool IsHybridMP4Format(const char *format)
{
	return format && strcmp(format, HYBRID_MP4_FORMAT) == 0;
}

static inline const char *RecordingFormatExtension(const char *format)
{
	return IsHybridMP4Format(format) ? "mp4" : format;
}

static inline const char *StandardRecordingOutputId(const char *format)
{
	return IsHybridMP4Format(format) ? "mp4_output" : "ffmpeg_muxer";
}

static void ShowStartRecordingFailed(OBSBasic *main, QString reason,
				     bool hybridMP4)
{
	if (hybridMP4) {
		reason += "\n\n";
		reason += QTStr("Output.StartFailedHybridMP4Fallback");
	}

	QMessageBox::critical(main, QTStr("Output.StartRecordingFailed"),
			      reason);
}
...
SimpleOutput::SimpleOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	if (!ffmpegOutput) {
		...
		const char *recFormat = config_get_string(
			main->Config(), "SimpleOutput", "RecFormat");
		const char *outputId = StandardRecordingOutputId(recFormat);
		bool hybridMP4 = IsHybridMP4Format(recFormat);
		fileOutput = obs_output_create(outputId, "simple_file_output",
					       nullptr, nullptr);
		if (!fileOutput) {
			blog(LOG_ERROR,
			     "Failed to create %s recording output "
			     "(simple output)",
			     outputId);
			if (!hybridMP4)
...
bool SimpleOutput::ConfigureRecording(bool updateReplayBuffer)
{
	...
	const char *formatExt = RecordingFormatExtension(format);
	...
	if (updateReplayBuffer) {
		...
		strPath = GetOutputFilename(path,
					    ffmpegOutput ? "avi" : formatExt,
					    noSpace, overwriteIfExists,
					    f.c_str());
		obs_data_set_string(settings, "directory", path);
		obs_data_set_string(settings, "format", f.c_str());
		obs_data_set_string(settings, "extension", formatExt);
...
{
	bool hybridMP4 = IsHybridMP4Format(config_get_string(
		main->Config(), "SimpleOutput", "RecFormat"));
	if (!fileOutput) {
		blog(LOG_ERROR,
		     "Hybrid MP4 recording output is unavailable "
		     "(simple output)");
		ShowStartRecordingFailed(
			main, QTStr("Output.StartFailedGeneric"), hybridMP4);
		return false;
	}
	...
	if (!obs_output_start(fileOutput)) {
		...
		ShowStartRecordingFailed(main, error_reason, hybridMP4);
		return false;
	}
...
{
	return fileOutput && obs_output_active(fileOutput);
}
...
AdvancedOutput::AdvancedOutput(OBSBasic *main_) : BasicOutputHandler(main_)
{
	...
	if (ffmpegOutput) {
		...
		const char *outputId = StandardRecordingOutputId(recFormat);
		bool hybridMP4 = IsHybridMP4Format(recFormat);
		fileOutput = obs_output_create(outputId, "adv_file_output",
					       nullptr, nullptr);
		if (!fileOutput) {
			blog(LOG_ERROR,
			     "Failed to create %s recording output "
...
bool AdvancedOutput::SetupStreaming(obs_service_t *service)
{
	...
	if (!useStreamEncoder ||
	    (!ffmpegOutput && (!fileOutput || !obs_output_active(fileOutput)))) {
		UpdateStreamSettings();
	}
...
bool AdvancedOutput::StartRecording()
{
	...
	bool hybridMP4 = false;

	if (!ffmpegOutput) {
		hybridMP4 = IsHybridMP4Format(config_get_string(
			main->Config(), "AdvOut", "RecFormat"));
		if (!fileOutput) {
			blog(LOG_ERROR,
			     "Hybrid MP4 recording output is unavailable "
			     "(advanced output)");
			ShowStartRecordingFailed(
				main, QTStr("Output.StartFailedGeneric"),
				hybridMP4);
			return false;
		}
	}
		...
		configuredFormat =
			config_get_string(main->Config(), "AdvOut",
					  ffmpegRecording ? "FFExtension"
							  : "RecFormat");
		recFormat = ffmpegRecording
				    ? configuredFormat
				    : RecordingFormatExtension(configuredFormat);
...
BasicOutputHandler::GetRecordingFilename(const char *path, const char *ext,
					 ...
					 const char *format, bool applyAutoRemux)
{
	if (applyAutoRemux)
		SetupAutoRemux(ext);
