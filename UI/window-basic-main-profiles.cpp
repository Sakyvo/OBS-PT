			  ...
			  const char *init_text, bool rename)
{
	...
	/* OBS-PT ships a complete PotPvP profile template and does not use
	 * upstream's auto-configuration wizard for new profiles. */
	bool showWizardChecked = false;

	if (!AskForProfileName(this, name, title, text, false,
			     ...
			     bool showWizardChecked, bool rename)
{
	...
	if (create_new) {
		if (!CopyProfile("PotPvP", newPath.c_str())) {
			blog(LOG_WARNING,
			     "Failed to copy OBS-PT PotPvP profile template");
		}
	} else {
	...
	/* OBS-PT: upstream auto-config is intentionally disabled. New clean
	 * profiles are seeded from the PotPvP profile template instead. */
