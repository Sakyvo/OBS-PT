	...
	try {
...

		{
			...
			get_portable_path(install_root, sizeof(install_root),
					  "");
			write_probe_result_t probe =
				probe_install_root_writable(install_root);
			if (probe != WRITE_PROBE_OK) {
				ShowWritePermissionFailureDialog(probe);
				return 1;
			}

			char logs_dir[512];
			char crashes_dir[512];
			if (get_portable_path(logs_dir, sizeof(logs_dir),
					      "obs-studio/logs") > 0)
				sweep_retention(logs_dir, 30, "*.txt",
						nullptr);
			if (get_portable_path(crashes_dir,
					      sizeof(crashes_dir),
					      "obs-studio/crashes") > 0)
				sweep_retention(crashes_dir, 30, "*.log",
						nullptr);
		}
...
{
	return get_portable_path(path, size, name);
}

char *GetConfigPathPtr(const char *name)
{
	return get_portable_path_ptr(name);
}
